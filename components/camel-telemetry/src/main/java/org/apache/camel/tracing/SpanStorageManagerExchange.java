...
public class SpanStorageManagerExchange implements SpanStorageManager {
    ...
    public Span peek(Exchange exchange) {
        SpanWrapper wrap = exchange.getProperty(ACTIVE_SPAN, SpanWrapper.class);
        if (wrap == null) {
            return null;
        }
        return wrap.current;
    ...
    public Span pull(Exchange exchange) {
        ...
        if (wrap.parent != null) {
            // Restore the parent as the current one
            push(exchange, wrap.parent);
        }
    ...
    public void push(Exchange exchange, Span span) {
        SpanWrapper parentWrap = exchange.getProperty(ACTIVE_SPAN, SpanWrapper.class);
        Span parentSpan = null;
        if (parentWrap != null) {
            parentSpan = parentWrap.current;
        }
        SpanWrapper currentWrap = new SpanWrapper(span, parentSpan);
    ...
    class SpanWrapper {
        ...
        public SpanWrapper(Span current, Span parent) {
