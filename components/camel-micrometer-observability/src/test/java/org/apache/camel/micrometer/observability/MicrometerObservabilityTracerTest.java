...
public class MicrometerObservabilityTracerTest extends MicrometerObservabilityTracerTestSupport {
    ...
    void testRouteSingleRequest() throws IOException {
        ...
        System.out.println("****" + result.getIn().getHeaders());
        checkTraces();
    }
    ...
    private void checkTraces() {
        ...
        Assertions.assertThat(registry)
                .hasAnObservation(t -> {
                    ...
                    t.doesNotHaveParentObservation();
                });
        Assertions.assertThat(registry)
                .hasAnObservation(t -> {
                    t.hasNameEqualTo("start");
                    t.hasHighCardinalityKeyValue("op", Op.EVENT_RECEIVED.toString());
                    t.hasParentObservationContextMatching(obj -> {
                        Observation.Context parentContext = (Observation.Context) obj;
                        return parentContext.getName().equals("start") &&
                                parentContext.getHighCardinalityKeyValue("op").getValue().equals(Op.EVENT_SENT.toString());
                    });
                });
        Assertions.assertThat(registry)
                .hasAnObservation(t -> {
                    t.hasNameEqualTo("log");
                    t.hasParentObservationContextMatching(obj -> {
                        Observation.Context parentContext = (Observation.Context) obj;
                        return parentContext.getName().equals("start") &&
                                parentContext.getHighCardinalityKeyValue("op").getValue().equals(Op.EVENT_RECEIVED.toString());
                    });
                });

        // Verify completion
        Assertions.assertThat(registry)
                .hasObservationWithNameEqualTo("start")
                .that()
                .hasBeenStarted()
                .hasBeenStopped();
        Assertions.assertThat(registry)
        ...
        // Verity the logging event --> will be available after version 1.15.0
        // Assertions.assertThat(registry)
        //     .hasObservationWithNameEqualTo("log")
        //     .that()
        //     .hasEvent(...);
